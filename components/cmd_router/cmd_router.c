...
int set_sta(int argc, char **argv)
{
    ...
    if (err == ESP_OK) {
        ...
        if (err == ESP_OK) {
            err = nvs_commit(nvs);
            if (err == ESP_OK) {
                printf(TAG, "STA settings %s/%s stored.", set_sta_arg.ssid->sval[0], set_sta_arg.password->sval[0]);
            }
        }
    }
    nvs_close(nvs);
...
int set_ap(int argc, char **argv)
{
    ...
    if (err == ESP_OK) {
        ...
        if (err == ESP_OK) {
            err = nvs_commit(nvs);
            if (err == ESP_OK) {
                printf(TAG, "AP settings %s/%s stored.", set_ap_args.ssid->sval[0], set_ap_args.password->sval[0]);
            }
        }
    }
    nvs_close(nvs);
