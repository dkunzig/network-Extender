...
static esp_err_t store_blob(nvs_handle_t nvs, const char *key, const char *str_values)
{
    ...
    if (str_len % 2) {
        printf(TAG, "Blob data must contain even number of characters");
    ...
    for (int i = 0, j = 0; i < str_len; i++) {
        ...
        if (ch >= '0' && ch <= '9') {
            ...
            printf(TAG, "Blob data contain invalid character");
            free(blob);
            return ESP_ERR_NVS_TYPE_MISMATCH;
        }
...
static esp_err_t set_value_in_nvs(const char *key, const char *str_type, const char *str_value)
{
    ...
    if (type == NVS_TYPE_ANY) {
        printf(TAG, "Type '%s' is undefined", str_type);
        return ESP_ERR_NVS_TYPE_MISMATCH;
    }
    ...
    if (err == ESP_OK) {
        err = nvs_commit(nvs);
        if (err == ESP_OK) {
            printf(TAG, "Value stored under key '%s'", key);
        }
    }
...
static esp_err_t erase(const char *key)
{
    ...
    if (err == ESP_OK) {
        ...
        if (err == ESP_OK) {
            err = nvs_commit(nvs);
            if (err == ESP_OK) {
                printf(TAG, "Value with key '%s' erased", key);
            }
        }
        nvs_close(nvs);
    }
...
static esp_err_t erase_all(const char *name)
{
    ...
    printf(TAG, "Namespace '%s' was %s erased", name, (err == ESP_OK) ? "" : "not");
...
static int list(const char *part, const char *name, const char *str_type)
{
    ...
    if (it == NULL) {
        printf(TAG, "No such enty was found");
        return 1;
    }
...
static int set_value(int argc, char **argv)
{
    ...
    if (err != ESP_OK) {
        printf(TAG, "%s", esp_err_to_name(err));
        return 1;
    }
...
static int set_namespace(int argc, char **argv)
{
    ...
    printf(TAG, "Namespace set to '%s'", current_namespace);
    return 0;
}
