...
static esp_err_t wifi_event_handler(void *ctx, system_event_t *event)
{
  switch(event->event_id) {
    ...
    case SYSTEM_EVENT_STA_GOT_IP:
        ap_connect = true;
        printf("got ip:" IPSTR, IP2STR(&event->event_info.got_ip.ip_info.ip));
    ...
    case SYSTEM_EVENT_AP_STACONNECTED:
        connect_count++;
        printf(TAG,"%d. station connected", connect_count);
        break;
    case SYSTEM_EVENT_AP_STADISCONNECTED:
        connect_count--;
        printf(TAG,"station disconnected - %d remain", connect_count);
        break;
    default:
        break;
  }
...
//    printf(TAG, "DNS IP:" IPSTR, IP2STR(&dnsinfo.ip.u_addr.ip4));
    ...
    if (strlen(ssid) > 0) {
        printf(TAG, "wifi_init_apsta finished.");
        printf(TAG, "connect to ap SSID: %s ", ssid);
    } else {
        printf(TAG, "wifi_init_ap with default finished.");      
    }
}
...
void app_main(void)
{
    ...
    printf(TAG, "Command history enabled");
#else
    printf(TAG, "Command history disabled");
#endif
    ...
    if (strcmp(lock, "0") ==0) {
        printf(TAG,"Starting config web server");
        start_webserver();
    }
    free(lock);
